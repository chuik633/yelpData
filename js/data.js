...
const cuisines = [
  "Italian", "Chinese", "Mexican", "Indian", "Japanese", "French", "American",
  "Thai", "Spanish", "Korean", "Vietnamese", "Mediterranean", "Middle Eastern",
  "Greek", "African", "Turkish", "Caribbean", "Cajun", "Soul Food", "Vegan", "Vegetarian"
];
const fetchBusinessData = async () =>{
  const business_data = await d3.json('./data/yelp_dataset/business_reformatted.json').then(
    (data)=>{
      //parse the data
      ...
      let filteredData = parsedData.filter(
       ...
       (d.categories.some(category => category.toLowerCase().includes("food")) 
       ||
        d.categories.some(category => category.toLowerCase().includes("restaurant"))))
      return  filteredData
    }
  )
...
const fetchReviewData = async() =>{
  const review_data = await d3.json('./data/yelp_dataset/review_reformatted.json')
...
function data1_protein(business_data, review_data){
  ...
  const proteinKeywords = {
    "Chicken": ["chicken", "poultry", "grilled chicken", "fried chicken", "chicken breast", "chicken wings"],
    "Beef": ["beef", "steak", "ground beef", "roast beef", "sirloin", "ribeye", "burger", "steakhouse"],
    "Pork": ["pork", "pork chops", "pulled pork", "sausage", "bacon", "ham", "ribs"],
    "Fish": ["fish", "salmon", "tuna", "cod", "trout", "seafood", "sashimi", "sea bass"],
    "Tofu": ["tofu", "soy", "tempeh", "soybean", "vegetarian protein", "vegan protein"],
    ...
    "Egg": ["egg", "eggs", "scrambled eggs", "fried eggs", "boiled eggs", "omelet", "egg whites"],
    "Shrimp": ["shrimp", "prawn", "shrimp cocktail", "shrimp scampi", "shrimp skewers"]
  };
  ...
  const cuisineInfoData = business_data.map(item=>{
    ...
    for(const cuisine of cuisines){
      if(item.categories.some(category => category.toLowerCase() ==cuisine.toLowerCase())){
  ...
  const businessCuisineData = cuisineInfoData.filter(item => item != undefined).flatMap(
    item=>item['cuisines'].map(
      cuisine =>({
  ...
  const reviewProteinData = review_data.filter(item=> item.text != undefined).map(
    item=>{
      return {
        business_id: item.business_id,
  ...
  reviewProteinData.forEach(item =>{businessIDToProtein.set(item.business_id, item.protein)})
  ...
  const cuisineProteinData = d3.rollup(
    businessCuisineData,
    (v)=>v.length,
    (d) => {return d.cuisine}, //group by the cuisine
    (d) => {
  ...
  const fcuisineProteinData =  Array.from(cuisineProteinData).map(entry => {
    entry[1] = Array.from(entry[1]).filter(innerEntry => innerEntry[0]!= 'no_match')
    return entry
...
//2. Review Sentiments
 ...
 const theme_keywords = {
  ...
  "Accessibility": ["access", "location", "parking", "wheelchair", "easy to find"],
  ...
  "Food Variety": ["menu", "variety", "options", "choices", "vegan", "gluten-free"]
}
...
function data2_sentiments(data){
  ...
  const sentimentThemeData = filteredData.map(
    (d) =>{
      return {
  ...
  const flattenedThemeData = sentimentThemeData.flatMap(
    item=>item['themes'].map(
      theme =>({
        ...item,
        theme:theme
      })
...
function data3_openTimes(data){
  ...
  for(const day of daysOfWeek){
      for(let hour=0;hour<24;hour++){
       ...
       allDayTimeData[day][hour] = Object.fromEntries(d3.rollup(allDayTimeData[day][hour],
            v=>{
                return{
                  count:v.length,
            ...
            d=> `${Math.floor(d.latitude/roundingVal)*roundingVal} ${Math.floor(d.longitude/roundingVal)*roundingVal}`
        ) )
      }
  } 
...
function data4_cuisines(business_data){
  ...
  let caBusinesses = business_data.filter(d=>isInCalifornia(d.latitude, d.longitude))
  const cities = Array.from(new Set(business_data.map(d=>d.city)))
  caBusinesses = business_data
  const flattenedCuisinesData = caBusinesses.flatMap(
    item => item['categories'].map( 
      category=>({
        ...item,
  ...
  const cuisineLocationData = d3.group(
    cuisineFilteredData,
    d=> d.cuisine
  )
...
export {fetchBusinessData, fetchReviewData, data1_protein, data2_sentiments, data3_openTimes, data4_cuisines,get_sentiment,theme_keywords}
